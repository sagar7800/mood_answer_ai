...
function showHistoryDetail(index) {
  ...
  if (detailDeleteButton) {
    detailDeleteButton.dataset.historyIndex = String(index);
  }
...
async function deleteHistoryAtIndex(index) {
  const item = savedHistory[index];
  ...
  const confirmed = window.confirm("Delete this saved answer from history?");
  ...
  try {
    await MoodAnswerApi.deleteHistoryItem(item.id);
    savedHistory.splice(index, 1);
    activeHistoryIndex = -1;
    historyDetail.hidden = true;
    renderQuestionList(savedHistory);
  } catch (error) {
    window.alert(error.message || "Could not delete this history item.");
  }
}
...
function renderQuestionList(items) {
  ...
  items.forEach((item, index) => {
    ...
    row.className = "history-question-item";
    ...
    deleteButton.className = "history-delete-button history-delete-inline";
    deleteButton.type = "button";
    deleteButton.textContent = "Delete";
    deleteButton.setAttribute(
      "aria-label",
      `Delete saved question: ${item.question || "Untitled question"}`,
    );
    deleteButton.addEventListener("click", () => deleteHistoryAtIndex(index));

    row.append(button, deleteButton);
    historyQuestionList.append(row);
  });
...
if (detailDeleteButton) {
  detailDeleteButton.addEventListener("click", () => {
    deleteHistoryAtIndex(Number(detailDeleteButton.dataset.historyIndex));
  });
}
